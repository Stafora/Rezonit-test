<template>
	<div class="adding-board-params-error-panel" v-if="vForm.isPanel.$invalid || vForm.maxWidth.$invalid || vForm.maxHeight.$invalid">
		...
		<div class="adding-board-params-error-panel-text">
			...
			<div v-if="vForm.isPanel.$invalid">
				<template v-if="!vForm.isPanel.millingAndScribing || !vForm.milling.notNull && !vForm.scribing.notNull">
					Фрезерование или Скрайбирование должно быть заполнено обязательно<br>
				</template>
			</div>

            <div v-if="vForm.maxWidth.$invalid">
				<template v-if="!vForm.maxWidth.maxWidthPanel">
					Платы по ширине не вместятся в лист, ограничение 400 мм
				</template>
			</div>

            <div v-if="vForm.maxHeight.$invalid">
				<template v-if="!vForm.maxHeight.maxHeightPanel">
					Платы по высоте не вместятся в лист, ограничение 600 мм
				</template>
			</div>
